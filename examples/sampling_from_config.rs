...
fn main() {
    let model = std::env::args()
        .nth(1)
        .expect("usage: sampling_from_config <model.gguf>");
    ...
    println!(
        ...
        settings.sampling_for(&model)
    );
    println!(
        "configured max_new_tokens:  {:?}",
        settings.max_new_tokens_for(&model)
    );
    ...
    let none = c
        .send("Reply with exactly: OK", Some(8), None, None, None, None)
        .expect("send with None");
    ...
    let long = c
        .send(
            "Count from 1 to 30, one number per line, nothing else.",
            None,
            None,
            None,
            None,
    ...
    println!(
        "max_new_tokens: None -> {} tokens, {lines} lines",
        long.tokens_generated
    );
    assert!(
        long.tokens_generated > 8,
        "a turn with no cap must not be limited to the 8 used above"
    );
