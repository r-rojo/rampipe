...
fn main() -> Result<()> {
...

    {
        ...
        conversation
            .save_state(&state_path, &meta_path)
            .context("save_state")?;
    ...
    let session = LlamaSession::load(&registry, backend, &model_path, Residency::Lazy)
        .context("reloading session")?;
    let mut conversation = session
        .open_conversation_from_state(&state_path, &meta_path)
        .context("open_conversation_from_state")?;
    ...
    if !turn2.text.contains("7492") {
        bail!(
            "turn 2 (after a disk save/reload) didn't recall the number from turn 1 -- the KV cache round-trip isn't actually working: {:?}",
            turn2.text
        );
    }
