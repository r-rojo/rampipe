...
fn main() -> Result<()> {
    ...
    let socket_path = PathBuf::from(args.next().context("usage: rampiped_generate_smoke <socket-path>")?);
    ...
    println!("  time to first token: {}ms", outcome.time_to_first_token_ms);
    ...
    if !outcome.text.to_lowercase().contains("paris") {
        bail!("expected the reply to mention Paris, got {:?}", outcome.text);
    }
